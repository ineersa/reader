...
- `castor dev:up` and `castor dev:restart` stop conflicting running containers from other compose projects if they already bind the configured dev ports (`HTTP_PORT`, `HTTPS_PORT`).
